...
UrlRedirectHandler::UrlRedirectHandler(std::string&& short_url,
				       db::ShortenedUrlsDatabase *db, const app_config::ReadOnlyAppConfig *const ro_app_config)
...
void UrlRedirectHandler::onRequest(
  ...
  DLOG_IF(INFO, req->getMethod() != proxygen::HTTPMethod::GET) << "Request handler factory should ensure this is a GET before creating `UrlRedirectHandler`";
  DLOG_IF(INFO, short_url_ != url_shortening::parse_out_request_str(req->getPathAsStringPiece())) << "Incorrect `short_url` passed to `UrlRedirectHandler` constructor";
  ...
  std::move(f).via(evb).thenTry([this](folly::Try<bool> result) mutable {
	...
	.header(proxygen::HTTPHeaderCode::HTTP_HEADER_LOCATION, std::move(buf_))
	.sendWithEOM();
    } else {
...
void UrlRedirectHandler::onBody(std::unique_ptr<folly::IOBuf> /*body*/) noexcept {
  // nop
...
void UrlRedirectHandler::onError(
