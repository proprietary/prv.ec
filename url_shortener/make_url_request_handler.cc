...
void MakeUrlRequestHandler::onEOM() noexcept {
  ...
  query_captcha_service(user_captcha_response)
      .thenValue([this, evb, long_url](std::string result) mutable {
        ...
        if (json["ip"].asString().length() > 0 /* json["success"].asBool() */) {
          ...
          return folly::via(folly::getGlobalCPUExecutor(),
                            std::bind(&MakeUrlRequestHandler::do_shorten_url,
                                      this, long_url));
        } else {
  ...
  //  // TODO(zds): this is a synchronous DNS lookup! bad in an event loop!
  // addr.setFromHostPort(external_service_url.getHost(),
  // external_service_url.getPort()); request_headers_to_captcha_service_ =
  // std::make_unique<proxygen::HTTPMessage>();
  // request_headers_to_captcha_service_->setURL(external_service_url.getPath());
  // request_headers_to_captcha_service_->setQueryString(external_service_url.getQuery());
  // request_headers_to_captcha_service_->setMethod(proxygen::HTTPMethod::GET);
  // request_headers_to_captcha_service_->getHeaders().add(proxygen::HTTPHeaderCode::HTTP_HEADER_ORIGIN,
  // ro_app_config_->url_shortener_service_base_url);
  // request_headers_to_captcha_service_->getHeaders().add(proxygen::HTTPHeaderCode::HTTP_HEADER_HOST,
  // external_service_url.getHost());
  ...
  // ro_app_config_->server_user_agent);
  // request_headers_to_captcha_service_->getHeaders().add(proxygen::HTTPHeaderCode::HTTP_HEADER_CONTENT_TYPE,
  // "application/x-www-form-urlencoded");
  // request_headers_to_captcha_service_->setHTTPVersion(1, 1);
  // // See: https://developers.google.com/recaptcha/docs/verify
  // std::string captcha_service_request_body =
  ...
  // request_body_to_captcha_service_ =
  // folly::IOBuf::copyBuffer(captcha_service_request_body.data(),
  // captcha_service_request_body.size()); auto evb =
  ...
  //   std::inserter(next_proto_list, next_proto_list.begin()));
  //   ssl_context_->setAdvertisedNextProtocols(next_proto_list);
  //   connector_.connectSSL(evb, addr, ssl_context_, nullptr,
  //   std::chrono::milliseconds(1000), opts, folly::AsyncSocket::anyAddress(),
  //   external_service_url.getHost());
  // } else {
  //   connector_.connect(evb, addr, std::chrono::milliseconds(1000), opts);
  // }
  ...
  //     folly::SemiFuture<std::string> fut =
  //     db_put_result.getSemiFuture().via(executor);
  //     std::move(f).thenValue([this, evb](std::string &&_short_url) mutable {
  // 	evb->runInEventBaseThread([this, short_url = std::move(_short_url)]()
  // mutable { 	  proxygen::ResponseBuilder(downstream_).status(200,
  // "OK").body(std::move(short_url)).sendWithEOM();
  // 	});
  //     });
  //     // put shortened url into database in thread pool, so as to not block
  //     the event loop executor->add([this, long_url, db_put_promise =
  //     std::move(db_put_result)]() mutable {
  // 	auto generated_short_url =
  // url_shortening::generate_shortened_url(long_url,
  // ro_app_config_->highwayhash_key);
  //       db_->put(generated_short_url, long_url);
  // 	// TODO(zds): check and handle errors
  // 	db_put_promise.setValue(generated_short_url);
  //     });
  //   } else {
  //     proxygen::ResponseBuilder(downstream_).status(400, "Bad
  //     Request").sendWithEOM();
  //   }
  // };
}
