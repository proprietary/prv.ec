...
std::unique_ptr<folly::F14NodeMap<std::string, std::vector<uint8_t>>>
  ...
  for (const std::filesystem::directory_entry& dir_entry : std::filesystem::recursive_directory_iterator{frontend_doc_root}) {
    if (std::filesystem::is_regular_file(dir_entry.path())) {
      DLOG(INFO) << "in recursive directory crawl, visting file: " << dir_entry.path();
      // read full file into memory
      auto abspath = std::filesystem::absolute(dir_entry.path());
      struct stat statbuf;
      stat(abspath.c_str(), &statbuf);
      DLOG(INFO) << "Reading \"" << abspath.c_str() << "\" (" << statbuf.st_size << " bytes) into memory";
      std::vector<uint8_t> bytes(statbuf.st_size, 0U);
      std::unique_ptr<FILE, decltype(&fclose)> fh {fopen(abspath.c_str(), "rb"), &fclose};
      fread(bytes.data(), statbuf.st_size, sizeof(uint8_t), fh.get());
      auto file_identifier = std::filesystem::relative(abspath, frontend_doc_root).string();
...
void FrontendHandler::onRequest(std::unique_ptr<proxygen::HTTPMessage> request) noexcept {
  // assuming everything was checked upstream
  DLOG_IF(ERROR, request->getMethod() != proxygen::HTTPMethod::GET) << "FrontendHandler only accepts GET requests. Must be checked before invoking FrontendHandler.";
  ...
  std::string_view path (request->getPathAsStringPiece().begin(), request->getPathAsStringPiece().end());
  if(path[0] != '/' || path.length() == 0 || path.length() > max_request_path_len) {
  ...
  if (cache_req == frontend_dir_cache_->end()) {
    DLOG(ERROR) << "Did not find path \"" << path_identifier << "\" in cache for frontend files";
    proxygen::ResponseBuilder(downstream_).status(404, "Not Found").sendWithEOM();
    return;
  }
  const std::vector<uint8_t>* buf = &cache_req->second;
  proxygen::ResponseBuilder(downstream_).status(200, "OK").body(folly::IOBuf::wrapBuffer(buf->data(), buf->size())).sendWithEOM();
}
...
void FrontendHandler::requestComplete() noexcept {
  delete this;
}
