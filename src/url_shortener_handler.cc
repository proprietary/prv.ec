...
UrlShortenerApiRequestHandler::UrlShortenerApiRequestHandler(
    db::ShortenedUrlsDatabase *db, const uint64_t *highwayhash_key)
    : db_(db), highwayhash_key_(highwayhash_key) {}

void UrlShortenerApiRequestHandler::onRequest(
    std::unique_ptr<proxygen::HTTPMessage> req) noexcept {
  DLOG(INFO) << "Request from " << req->getClientIP()
             << " for path: " << req->getPath();
...
void UrlShortenerApiRequestHandler::onEOM() noexcept {
  ...
  if (request_->getMethod() == proxygen::HTTPMethod::GET) {
    ...
    VLOG(1) << "request from " << request_->getClientIP()
            << " to shorten this long URL: " << request_body;
...
void UrlShortenerApiRequestHandler::onBody(
...
void UrlShortenerApiRequestHandler::onUpgrade(
    proxygen::UpgradeProtocol /* proto */) noexcept {
  // not applicable
}

void UrlShortenerApiRequestHandler::requestComplete() noexcept { delete this; }

void UrlShortenerApiRequestHandler::onError(
    proxygen::ProxygenError err) noexcept {
  DLOG(INFO) << "proxygen onError";
  delete this;
}

void UrlShortenerApiRequestHandler::sendError(
...
void UrlShortenerApiRequestHandler::sendErrorBadRequest(
    const std::string &what) noexcept {
