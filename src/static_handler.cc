...
namespace ec_prv {
  namespace url_shortener {
	...
	bool validate_static_file_request_path(std::string_view input) {
      ...
      void StaticHandler::onRequest(std::unique_ptr<proxygen::HTTPMessage> request) noexcept {
	...
	try {
	  file_ = std::make_unique<folly::File>(request->getPathAsStringPiece().subpiece(1));
	...
	folly::getUnsafeMutableGlobalCPUExecutor()->add(std::bind(&StaticHandler::read_file, this, folly::EventBaseManager::get()->getEventBase()));
      }
